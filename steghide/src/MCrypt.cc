...
MCrypt::MCrypt (EncryptionAlgorithm a, EncryptionMode m)
{
	open (a, m) ;
}
...
void MCrypt::open (EncryptionAlgorithm a, EncryptionMode m)
{
...
BitString MCrypt::encrypt (BitString p, std::string pp)
{
...
BitString MCrypt::decrypt (BitString c, std::string pp)
{
...
void* MCrypt::createKey (std::string pp)
{
...
std::vector<unsigned char> MCrypt::_encrypt (std::vector<unsigned char> p, std::string pp)
{
...
std::vector<unsigned char> MCrypt::_decrypt (std::vector<unsigned char> c, std::string pp)
{
	// generate key
	void *key = createKey (pp) ;
	unsigned char *IV = NULL ;
...
EncryptionAlgorithm MCrypt::getAlgorithm () const
{
	myassert (ModuleOpen) ;
...
EncryptionMode MCrypt::getMode () const
{
	myassert (ModuleOpen) ;
...
unsigned long MCrypt::getEncryptedSize (EncryptionAlgorithm a, EncryptionMode m, unsigned long plnsize)
{
...
std::vector<std::string> MCrypt::getListAlgorithms ()
{
	int size = 0 ;
...
std::vector<std::string> MCrypt::getListModes ()
{
	int size = 0 ;
...
bool MCrypt::AlgoSupportsMode (EncryptionAlgorithm a, EncryptionMode m)
{
	std::string tmp1 = a.getStringRep(), tmp2 = m.getStringRep() ;
	char algo[tmp1.size() + 1], mode[tmp2.size() + 1] ;
	strcpy (algo, tmp1.c_str()) ;
...
void *MCrypt::s_malloc (size_t size)
{
	void *retval = NULL ;
